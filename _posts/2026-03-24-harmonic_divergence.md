...
**Proof of Lemma.** Let $\sigma:\mathbb N \to \mathbb N$ be injective, $d\in\mathbb N$ be any natural number, and $n_1,\ldots,n_d$ be a finite collection of *distinct* numbers. We will begin by showing that the random vectors $\vec X_d := (X_{n_1},\ldots, X_{n_d})$ and $\vec Y_d := (X_{\sigma(n_1)},\ldots, X_{\sigma(n_d)})$ have the same distribution. To that end, let $R_d := [a_1,b_1]\times \cdots\times [a_d, b_d]$ be a "rectangle", and observe by the i.i.d. assumption on the $X_i$'s that
