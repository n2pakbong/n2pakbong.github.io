...
So we have shown that the cardinal of $R$ is almost surely infinite. In particular, its expectation is also infinite. Now, recall by the same symmetry argument from above that the event 

$$
\{n\in R\} = \{ X_n > X_j \text{ for all } j< n\} 
$$

has probability $1/n$ by invariance under permutation. By [Tonelli's theorem](https://en.wikipedia.org/wiki/Fubini's_theorem#Tonelli's_theorem_for_non-negative_measurable_functions) for non-negative measurable functions, we can therefore exchange $\mathbb E$ and $\sum$ to conclude that
...
**Proof of Lemma.** We actually prove the lemma for any sequence of i.i.d. real-valued random variables. Let $\sigma:\mathbb N \to \mathbb N$ be injective, $d\in\mathbb N$ be any natural number, and $n_1,\ldots,n_d$ be a finite collection of *distinct* numbers. We will begin by showing that the random vectors $\vec X_d := (X_{n_1},\ldots, X_{n_d})$ and $\vec Y_d := (X_{\sigma(n_1)},\ldots, X_{\sigma(n_d)})$ have the same distribution. To that end, let $R_d := [a_1,b_1]\times \cdots\times [a_d, b_d]$ be a "rectangle", and observe by the i.i.d. assumption on the $X_i$'s that
...
Now it is clear that the collection $\mathcal R_d$ of all $d$-dimensional closed hyper-rectangles forms a [$\pi$-system](https://en.wikipedia.org/wiki/Pi-system) (i.e., is closed under finite intersections). Furthermore, (the algebra generated by) this collection is a subset of the collection $\mathcal A$ of measurable sets $A$ such that $\mathbb P(\vec X_d \in A) =\mathbb P(\vec Y_d \in A)$. By the [continuity from above and below of probability measures](https://en.wikipedia.org/wiki/Measure_(mathematics)#Continuity_from_below), $\mathcal A$ is easily seen to be a [monotone class](https://en.wikipedia.org/wiki/Monotone_class_theorem#Definition_of_a_monotone_class), and therefore the [monotone class theorem](https://en.wikipedia.org/wiki/Monotone_class_theorem) yields:

$$
\sigma(\mathcal R_d) \subseteq \mathcal A,
$$

where $\sigma(\mathcal R_d)$ denotes the [$\sigma$-algebra generated by](https://en.wikipedia.org/wiki/%CE%A3-algebra#%CF%83-algebras_generated_by_families_of_sets) $\mathcal R_d$. It is immediate that the closed [rectangles generate the whole Borel $\sigma$-algebra](https://en.wikipedia.org/wiki/Borel_set) and so we have established that $\vec X_d := (X_{n_1},\ldots, X_{n_d})$ and $\vec Y_d := (X_{\sigma(n_1)},\ldots, X_{\sigma(n_d)})$ have the same distribution. Clearly, this holds for all choices of $d\in\mathbb N$ and disjoint $n_1,\ldots,n_d\in\mathbb N$.  

Now that we've shown that the desired claim holds for finite-dimensional projections, all that's left is to remember that [the cylindrical $\sigma$-algebra](https://en.wikipedia.org/wiki/Cylindrical_%CF%83-algebra) generates the Borel $\sigma$-algebra on $\mathbb{R}^{\mathbb{N}}$. Indeed, with that fact in hand, we can essentially repeat the argument we gave previously: the cylinders form a $\pi$-system and the algebra they generate is a subset of the sets where the two distributions agree. A last application of the monotone class theorem thus yields that the sigma-algebra they generate is still a subset of the set where the two measures agree, so we are done.  $\square$
